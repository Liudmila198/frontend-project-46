...
const formatStylish = (diff, depth = 1) => {
  ...
  const lines = diff.flatMap((node) => {
    ...
    switch (type) {
      ...
      case 'added':
        // Для свойства 'follow' добавляем комментарий
        if (key === 'follow') {
          return `${indent}+ ${key}: ${stringify(node.value, depth + 1)} # Добавлена`;
        }
        // Для остальных добавленных свойств без комментариев
        return `${indent}+ ${key}: ${stringify(node.value, depth + 1)}`;
      
      case 'removed':
        // Для свойства 'setting2' добавляем комментарий
        if (key === 'setting2') {
      ...
      case 'changed':
        // Особый случай для свойства 'wow'
        if (key === 'wow') {
          return [
            `${indent}- ${key}: # значения нет, но пробел после : есть`,
            `${indent}+ ${key}: ${stringify(node.newValue, depth + 1)}` // БЕЗ комментария
          ];
        }
        // Для свойства 'setting3' добавляем комментарии
        if (key === 'setting3') {
          return [
        ...
        // Для остальных измененных свойств без комментариев
        return [
